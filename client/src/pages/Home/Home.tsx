...
export default function Home() {
  ...
  const filterByDate = (dateStr: string) => {
    ...
    if (filterType === 'today') return targetTime === nowTime;
    if (filterType === 'week') {
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto px-6 py-8">
        <div className="lg:col-span-2 space-y-10">
          ...
          {activeGroups.filter((g: any) => g.phase !== "crescimento").length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={() => setLocation("/groups")}>Ver Todos</Button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {activeGroups
                  ...
                  .map((group: any) => {
                    ...
                    return (
                      <Card key={group.id} className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => activeBatch ? setLocation(`/batches/${activeBatch.id}`) : setLocation(`/groups/${group.id}`)}>
                        <CardHeader>
                        ...
                        <CardContent>
                          <div className="space-y-3">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="font-semibold text-primary">{formatQuantity(group.quantity)}</span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-sm text-muted-foreground">Fase:</span>
                              <span className="px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800">Postura</span>
                            </div>
                            <div className="flex justify-between items-center">
          ...
          {/* Recent Groups (Crescimento) */}
          {activeGroups.filter((g: any) => g.phase === "crescimento").length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-foreground">Caixas de Crescimento Recentes</h2>
                <Button variant="outline" size="sm" onClick={() => setLocation("/groups/growth")}>Ver Todas</Button>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {activeGroups
                  .filter((g: any) => g.phase === "crescimento")
                  ...
                  .map((group: any) => {
                    const activeBatch = (batches || []).find(b => b.galpao_id === group.id && b.status === 'active');
                    return (
                      <Card key={group.id} className="hover:shadow-lg transition-shadow cursor-pointer border-l-4 border-l-blue-500" onClick={() => activeBatch ? setLocation(`/batches/${activeBatch.id}`) : setLocation(`/groups/${group.id}`)}>
                        <CardHeader>
                          <CardTitle className="text-lg">{group.name}</CardTitle>
                          <CardDescription>{group.species}</CardDescription>
                        </CardHeader>
                        <CardContent>
                          <div className="space-y-3">
                            ...
                            <div className="flex justify-between items-center">
                              <span className="text-sm text-muted-foreground">Fase:</span>
                              <span className="px-2 py-1 rounded text-xs font-medium bg-blue-100 text-blue-800">Crescimento</span>
                            </div>
                            <div className="flex justify-between items-center">
                              <span className="text-sm text-muted-foreground">Local:</span>
                              <span className="text-sm font-medium">{group.location}</span>
      ...
      {safeGroups.length === 0 && (
        <div className="text-center py-12 bg-muted/50 rounded-lg">
          ...
          <p className="text-muted-foreground mb-6">Acompanhe a produção e gerencie os lotes ativos.</p>
        </div>
      )}
    </div>
  );
